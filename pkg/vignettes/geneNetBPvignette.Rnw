...
Fit a CG-BN to \code{hdl} data using\code{fit.gnbp}. To do this, load the data and discretize phenotypes around median. 
...
## Fit a CG-BN
cgbn<-fit.gnbp(hdlgeno,hdlpheno_dis,constraints=constraints,alpha=0.08)
@
...
As desired, HDL is downstream of the genes in the network. Note that is also possible to specify undirected edges (required or forbidden), however they will be ignored since the basis of our model is a directed acyclic graph.\\

For \code{fit.dbn}, the \code{whitelist} option can be used to specify the required edges while the \code{blacklist} option can be used to specify forbidden edges.
...
A graph structure can also be directly input to both the fit methods. The following example illustrates how to input a known graph structure using the \code{fit.dbn} function to fit a discrete bayesian network to HDL data. We inferred the 
...
Suppose the interactions between the variables in the \code{toy} dataset are known as shown in Section 2. There are two ways to enter this network structure in the fit methods: 1.specify a list of edges or 2. input a graph structure of class "graphNEL". In this section we will use the first approach with \code{fit.gnbp}
...
\begin{Schunk}
\begin{Sinput}
> network<-fit.gnbp(toygeno,toypheno,learn=FALSE,graph=edgelist)
> ##Generate evidence
