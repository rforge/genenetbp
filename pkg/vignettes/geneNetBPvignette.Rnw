...
\begin{figure}[H]
\centering 
\includegraphics{yeast_graph}
\caption{Discrete Bayesian Network learnt from Yeast data}
\end{figure} 
...
<<F,results=TRUE,eval=TRUE>>=
yeast.dbn<-fit.dbn(yeastgeno,yeastpheno_dis)
@

\begin{center}
<<F1,figs.only=TRUE,eval=TRUE>>=
plot(yeast.dbn)
@
\end{center}
