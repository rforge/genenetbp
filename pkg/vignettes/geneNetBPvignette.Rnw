...
For discrete bayesian networks inferred by \code{absorb.gnbp}
...
Fit a CG-BN to \code{hdl} data using\code{fit.gnbp}. To do this, load the data.  
...
For discrete bayesian network, discretize the phenotypes around median. To fit the network parameters, set \code{learn==FALSE} and specify the graph.\\
