...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 450 300 80,clip,height=7cm,width=7cm]{figures/toy_graph}
...
\begin{figure}[H]
\centering 
\includegraphics{figures/mouse_graph}
...
\begin{figure}[H]
\centering 
\includegraphics{figures/mouse_tlr12_evidence}
...
\noindent Belief propagation can be implemented in known genotype-phenotype networks. If the network structure is known apriori from a knowledge database, then learning step can be skipped in \code{fit.gnbp} by seting \code{learn = FALSE}. The conditional probabilities will still need to be learnt based on the input network. This section demonstrates how to specify known networks and subsequent belief propagation in a simulated toy example.\\
