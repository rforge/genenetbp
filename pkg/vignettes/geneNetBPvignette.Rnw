...
$marginal$geno$freq
         state1       state2       state3
Qchr4 0.9999584 1.677966e-05 2.486464e-05
...
## Plot the network
plot(yeast.dbn.abs,ncol=2,fontsize=14)
@

\begin{figure}[H]
\centering 
\includegraphics{yeast_db_cox10}
...
<<K,eval=FALSE>>=
plot.dbn(yeast.dbn.abs,y="FC",ncol=2,fontsize=14)
@
...
When \code{y="FC"} is specified, the fold changes for the specified column (evidence) are mapped on the network. FC are positive values. FC of 1 indicates no change from the marginal, greater than 1 indicates the updated mean is higher than the marginal mean while less than 1 indicates vice-versa.
