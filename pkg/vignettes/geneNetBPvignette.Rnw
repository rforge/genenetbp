...
If additional biological information is known, that can be provided as \code{constraints} in \code{fit.gnbp} and as \code{whitelist} and \code{blacklist} in \code{fit.dbn}. 

We will fit a CG-BN to \code{hdl} data using\code{fit.gnbp}. To do this, load the data and discretize phenotypes around median. 
...
A graph structure can also be directly input to both the fit methods. The following example illustrates this using the \code{fit.dbn} function to fit a discrete bayesian network to HDL data.
<<>>=
